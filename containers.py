    ...
    def dynamic_programming(self):
        ...
        for rows in range(self.num_containers):
            for column in range(rows, self.num_containers):
                ...
                print(f"Data-structure for containers is: {self.containers[rows:column + 1]}")
                print(f"For row: {rows} and column: {column}  with harbor-width: {self.width} and spacing: {self.spacing}")
                print(self.empty_space)


                self.row_cost[rows,column] = self.compute_row_cost(i = rows, j= column, empty_space=self.empty_space[rows, column])
                print(self.compute_row_cost(i = rows, j= column, empty_space=self.empty_space[rows, column]))
                print(self.row_cost)
